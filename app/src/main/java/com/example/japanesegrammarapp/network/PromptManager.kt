...
object PromptManager {
    ...
    val SYSTEM_PROMPT_TOKENIZER_IMAGE = """
        ...
        2. 読み取った文に不自然な歪みや文法的な誤り（手書きの崩れ、不鮮明な文字、濁点・半濁点の脱落、て・での混同など）がある場合は、日本語の文法および前後の文脈に従って、正しい文に自動的に修正してください。
        3. 認識・修正した後の正しい文全体の文字列を、JSONの "correctedText" フィールドに出力してください。
        4. "tokens" 配列には、その修正した後の【正しい文】を分かち書きしたトークンを出力してください。
        ...
        JSONスキーマ:
        {
          "correctedText": "画像から読み取った日本語テキストを、正しい文法・文脈になるよう修正した文字列（修正不要な場合は読み取り文と同一）",
          "tokens": ["トークン1", "トークン2", "トークン3"]
        }
        
        FEW-SHOT 参照例:
        画像内の文字が「図書館て本をよんている。」と認識される場合:
        期待されるJSONレスポンス:
        {
          "correctedText": "図書館で本を読んでいる。",
          "tokens": ["図書館", "で", "本", "を", "読んでいる", "。"]
